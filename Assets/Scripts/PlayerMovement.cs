...
public class PlayerMovement : MonoBehaviour
{
    ...
    private void RayCastGuns() {
        ...
        AntiGravGun gun1A = GameObject.Find("AntiGravGun").GetComponent<AntiGravGun>();
        RayCastPistol gun2P = GameObject.Find("Pistol").GetComponent<RayCastPistol>();
        //Временное решение проблемы с GunSway'ем
        GunSwayPistolEnabled = GameObject.Find("Pistol1").GetComponent<GunSway>();
        GunSwayAntiGravGunEnabled = GameObject.Find("SciFiGunHeavyWhite").GetComponent<GunSway>();
            ...
            switch (ObjectTag)
            {
                case "AntiGravGun":
                    ...
                    if (Input.GetKeyDown(KeyCode.E) && playerHand.childCount < 1)
                    {
                        ...
                        foreach (Transform transform in Item.GetComponentInChildren<Transform>(true))
                        { transform.gameObject.layer = 9; }
                        gun1A.gunready = true;
                        gun2P.gunready1 = false;

                        GunSwayAntiGravGunEnabled.enabled = true;
                        GunSwayPistolEnabled.enabled = false;
                        gunSwayReady = true;
